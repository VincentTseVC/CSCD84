...
void backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS])
{
  ...
  for (int i = 0; i < OUTPUTS; i++) {
    ...
    if (logistic == sigmoid) { // if its logistic function
      error = (target[i] - activations[i]) * (activations[i] * (1 - activations[i]));
    } else {
      error = (target[i] - activations[i]) * (1.0 - activations[i]*activations[i]);
    }
    for (int j = 0; j < INPUTS; j++)
...
void feedforward_2layer(double sample[INPUTS],
...
void backprop_2layer(double sample[INPUTS],
  double h_activations[MAX_HIDDEN],
  double activations[OUTPUTS],
  double (*sigmoid)(double input),
  int label,
  ...
  double weights_ho[MAX_HIDDEN][OUTPUTS], int units)
{
  ...
  for (int i = 0; i < OUTPUTS; i++) {
    if (logistic == sigmoid) {
      error = (target[i] - activations[i]) * (activations[i]*(1.0-activations[i]));
    } else {
      error = (target[i] - activations[i]) * (1.0 - activations[i]*activations[i]);
    }
    Derror[i] = error; // store the errors
  ...
  for (int i = 0; i < OUTPUTS; i++) {
    ...
    for (int k = 0; k < OUTPUTS; k++)
      error += Derror[k]*temp_weights_ho[i][k];

    if (logistic == sigmoid) {
      error = h_activations[i]*(1.0-h_activations[i]);
    } else {
      error = (target[i] - activations[i]) * (1.0 - h_activations[i]*h_activations[i]);
    }
